...
public class SQLHelper {
    ...
    //Метод, благодаря которому мы можем получить последний сгенерированный код
    @SneakyThrows
    public static String getVerificationCode() {
        var codeSQL = "SELECT code FROM auth_codes ORDER BY created DESC LIMIT 1";
        var conn = getConn();
        return QUERY_RUNNER.query(conn, codeSQL, new ScalarHandler<>());
        //ScalarHandler возвращает значение одного поля
    }
    ...
    //Метод для очистки таблицы с кодами для авторизации
    @SneakyThrows
